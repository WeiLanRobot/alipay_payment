...
class _AlipayDemoPageState extends State<AlipayDemoPage> {
  ...
  Future<void> _pay() async {
    ...
    await AlipayPaymentPlatform.instance.pay(orderInfo: orderInfo);
    // 结果通过 payResp 流返回，已在 initState 中监听
  }

  Future<void> _auth() async {
    const authInfo = 'test_auth_info_from_server';
    await AlipayPaymentPlatform.instance.auth(authInfo: authInfo);
      ...
      child: Padding(
        ...
        child: Column(
          ...
          children: [
            ...
            const SizedBox(height: 8),
            Text(
              'Scheme: 从 pubspec.yaml 配置（pod install 时注入）',
              style: const TextStyle(fontSize: 14),
            ),
          ],
        ),
